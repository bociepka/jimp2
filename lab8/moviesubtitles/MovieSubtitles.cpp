...
namespace moviesubs{
    ...
    void MicroDvdSubtitles::ShiftAllSubtitlesBy(int delay, int frame, std::stringstream *in, std::stringstream *out){
        char c;
        int lines_counter = 0;
        double double_delay = (double)delay/1000;
    ...
    void MicroDvdSubtitles::ChangeValues(std::string &line, int frames_to_add, int lines_counter) {
        ...
        std::string end_str = "";
        bool completed_start_value, completed_end_value = false;
        int open_bracket = 0;
    ...
    void SubRipSubtitles::ShiftAllSubtitlesBy(int delay, int frame, std::stringstream *in, std::stringstream *out) {
        ...
        while((*in).get(c)){
            ...
            if(end_of_line_counter==1){
                ...
                if(arrow_counter==5){
                    if(second.size()!=12 && first.size()==12){
                        second+=c;
                        current_line+=c;
                    }
                }else{
    ...
    std::string SubRipSubtitles::FromMiliseconds(long long int miliseconds_amount){
        ...
        if(miliseconds<100){
            if(miliseconds<10){
                answer+="00"+std::to_string(miliseconds);
            }
            else{
                answer+='0'+std::to_string(miliseconds);
            }
        }
        else{
            answer+=std::to_string(miliseconds);
        }
    ...
    std::string &SubRipSubtitles::TimeConverterToString(long long int hours, std::string &answer) {
        if (hours < 10){
            answer+='0' + std::__cxx11::to_string(hours);
        }
        else{
            answer+=std::__cxx11::to_string(hours);
        }
        answer+=':';
        return answer;
    }
